...
export async function createGame(mount, opts = {}) {
  // Load sound library if available
  let sound = opts.sound;
  if (!sound) {
    try {
      const globalSound =
        (typeof window !== "undefined" && window.PIXI && window.PIXI.sound) ||
        globalThis?.PIXI_SOUND;
      if (globalSound) {
        sound = globalSound;
      }
    } catch (e) {
      console.warn("Global PIXI sound lookup failed:", e.message);
    }
  }

  if (!sound) {
    console.warn("Sounds disabled: @pixi/sound package not available");
