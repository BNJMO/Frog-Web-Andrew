...
export class GameDesktopComponent implements OnInit {
  array = Array;
  ...
  messageDispatcher(message) {
    ...
    } else if (payload.type === 'player_bet') {
      msg.bets.forEach(b => {
        this.gameService.gameData.userBets.push(b);
      });
    } else if (payload.type === 'player_bet_cancelled') {
      msg.bets.forEach(b => {
        const bIndex = this.gameService.gameData.userBets.indexOf(this.gameService.gameData.userBets.find(x => x.betInfo.id === b.id));
        this.gameService.gameData.userBets.splice(bIndex, 1);
      });
